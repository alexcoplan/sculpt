module Sculpt
    class Sculpture < ElementContainer
        ...
        def generate_html
            return @elements.map(&:generate_html).join("\n") if Sculpt.pretty?
        ...
        def pre(text = '', attrs = {}, &block)
            text.unindent! if text.respond_to? :unindent
            add_tag Tag.new(:pre, text, attrs, self, &block)
        end
        ...
        def method_missing(method, *args, &block)
            ...
            if m.end_with? '_s'
                ...
                when :a
                    ...
                    tag = Tag.new(:a, arg1, attrs, self, &block)
                when :img
                    attrs = special_attr(:src, arg1, arg2)
                    tag = Tag.new(:img, attrs)
                else
                    tag = Tag.new(meth, arg1, arg2, self, &block)
                end
                return tag.generate_html
            end
            add_tag Tag.new(method, arg1, arg2, self, &block)
        end
    end
end